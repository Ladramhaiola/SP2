...
func testTable(t *Table) {
	...
	fmt.Printf("Insert 1: %s", t.insertDirect(newRecord("Feldman", 0, 89.0), 1).tostring())
	...
	fmt.Printf("Insert Key{Daragan, 90}: %s", t.insertLinear(newRecord("Daragan", 90, 13.3)).tostring())
	fmt.Printf("Update Key{Burbil, 9}: %s", t.updateLinear(Key{"Burbil", 9}, newRecord("Vel", 13, 0.3)).tostring())
	...
	fmt.Printf("Search 'Kuzay': %s", t.searchClosest("Kuzay").tostring())
}
